 ...
 * an underline and the script tagline beneath — echoing the layout of the
 * client's carved wood sign, built from real markup/fonts rather than a
 * flattened image so it stays crisp, translatable, and themeable.
 */
export default function BrandMark({
  ...
  const lineColor = tone === "dark" ? "bg-charcoal/25" : "bg-warm/30";
  ...
  return (
    <Link href={`/${locale}`} className="inline-block" data-cursor-hover>
      ...
      <span className={`mt-2 block h-px w-full max-w-[220px] ${lineColor}`} />
